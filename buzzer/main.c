...
/* Private typedef -----------------------------------------------------------*/
#define  PERIOD_VALUE  (uint32_t)(10000000/440 - 1)      // ARR Value (Period)
#define  PULSE_VALUE   (uint32_t)(PERIOD_VALUE*25/100) // Capture Compare Value (Duty Cycle)
...
{
    TIM12->ARR = (10000000UL / Frequency) - 1; // Set The PWM Frequency
    // TIM12->CCR1 = (TIM12->ARR >> 1); // Set Duty Cycle 50%
    HAL_Delay(Duration); // Wait For The Tone Duration
}

static void noTone()
{
    TIM12->CCR1 = 0; // Set Duty Cycle 0%
}
