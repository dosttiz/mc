/**
  ******************************************************************************
  * @file    Templates/lab05_distance/CM7/Src/main.c
...
int main(void)
{
 ...
 /* STM32H7xx HAL library initialization:
  ...
  USS_TRIG_GPIO_CLK_ENABLE();

  gpio_init_structure.Pin   = USS_TRIG_PIN;
  gpio_init_structure.Mode  = GPIO_MODE_OUTPUT_PP;
  gpio_init_structure.Pull  = GPIO_NOPULL;
  gpio_init_structure.Speed = GPIO_SPEED_FREQ_VERY_HIGH;

  HAL_GPIO_Init(USS_TRIG_GPIO_PORT, &gpio_init_structure);
...
/* Set TIMx instance */
  ...
  */
  ...
  if (HAL_TIM_IC_Start(&TimHandle, TIM_CHANNEL_4) != HAL_OK)
  {
    /* Starting Error */
    Error_Handler();
  }
  ...
  while (1)
  {
    ...
    HAL_GPIO_TogglePin(USS_TRIG_GPIO_PORT, USS_TRIG_PIN);

    HAL_Delay(100);

    uwDistance = RGB_Get_Dist();

    printf("DIST=%ld", uwDistance);
  }
}

uint32_t RGB_Get_Dist()
{
  __HAL_TIM_CLEAR_FLAG(&TimHandle, TIM_FLAG_CC4);
  __HAL_TIM_CLEAR_FLAG(&TimHandle, TIM_FLAG_CC4OF);

  uhCaptureIndex = 0;

  while (uhCaptureIndex < 2)
  {
    if (__HAL_TIM_GET_FLAG(&TimHandle, TIM_FLAG_CC4))
    {
      if (uhCaptureIndex == 0)
      {
      ...
      else if (uhCaptureIndex == 1)
      {
        ...
        // uwFrequency = 2*HAL_RCC_GetPCLK1Freq()/4 / uwDiffCapture;
        uwDistance = uwDiffCapture / 2 / 340;
